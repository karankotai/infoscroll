-- Update get_feed to exclude short_video cards (videos get their own separate feed)
...
BEGIN
  ...
  FROM (
    ...
    FROM (
      SELECT ucs.card_id, ucs.seen_at
    ...
    JOIN cards c ON c.id = recent_ucs.card_id
    GROUP BY c.topic
  ) recent;
...
-- New function: get_video_feed — returns only short_video cards, shuffled, excluding seen ones
CREATE OR REPLACE FUNCTION get_video_feed(p_user_id UUID, p_limit INT DEFAULT 20)
RETURNS SETOF cards
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
...
GRANT EXECUTE ON FUNCTION get_video_feed(UUID, INT) TO authenticated;
