-- Add short_video to card_type check constraint
...
ALTER TABLE cards ADD CONSTRAINT cards_card_type_check CHECK (card_type IN (
  'quick_fact', 'summary', 'mini_thread', 'key_insight', 'did_you_know', 'short_video'
));

-- Update pg_jsonschema constraint to include short_video
...
ALTER TABLE cards ADD CONSTRAINT valid_card_content CHECK (
  CASE card_type
    ...
    WHEN 'short_video' THEN extensions.jsonb_matches_schema(
      '{"type":"object","required":["youtube_id","channel_name","duration_seconds"],"properties":{"youtube_id":{"type":"string","minLength":1},"channel_name":{"type":"string","minLength":1},"duration_seconds":{"type":"integer","minimum":1,"maximum":60}}}',
      content
    )
    ELSE false
  END
